<?php 
    ...
    //pagination
     try{
     	...
     	$total1 = $pdo->query('SELECT COUNT(*) FROM lost')->fetchColumn();
     	$total2 = $pdo->query('SELECT COUNT(*) FROM found')->fetchColumn();
			...
			<?php
 			...
 			if(isset($_POST['sb'])){
				...
				// Prepare the paged query
 		   		$stmt = $pdo->prepare("
 		        SELECT * FROM lost WHERE nume LIKE '%" . $name ."%'ORDER BY id LIMIT :limit OFFSET :offset");
				...
				foreach ($iterator as $row) { ?>
 					...
 					<?php //if there is no image set
 						if($row['images'] == ""){ ?>								
 						<div class='announcement-image' style="background-image: url('lost-pictures/default.jpg')">
 									</div>
 			...
 			// Prepare the paged query
 		   		$stmt2 = $pdo->prepare("
 		        SELECT * FROM found WHERE nume LIKE '%" . $name ."%'ORDER BY id LIMIT :limit OFFSET :offset");
				...
				foreach ($iterator as $row) { ?>
 					...
 					<?php //if there is no image set
 						if($row['images'] == ""){ ?>								
 						<div class='announcement-image' style="background-image: url('found-pictures/default.jpg')">
 									</div>
